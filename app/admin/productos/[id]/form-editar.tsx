...
type Producto = {
  ...
  variantes: { tallaId: string; stock: number }[];
};
...
export function FormEditarProducto({
  ...
  const [requiereTalla, setRequiereTalla] = useState(producto.requiereTalla ?? true);
  ...
  for (const v of producto.variantes) {
    initialStock[v.tallaId] = v.stock;
  }
  const [stockPorTalla, setStockPorTalla] = useState<Record<string, number>>(initialStock);
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await actualizarProducto(producto.id, null, formData);
    setLoading(false);
  ...
  return (
    <form onSubmit={submit}>
      ...
      <div className="form-group">
        <label>Precio</label>
        <input name="precio" type="number" step="0.01" min="0" defaultValue={producto.precio} required />
      </div>
      <div className="form-group">
      ...
      <div className="form-group">
        <label>Color</label>
        <select name="colorId" defaultValue={producto.colorId ?? ""}>
          <option value="">Sin especificar</option>
          {colores.map((c) => (
            <option key={c.id} value={c.id}>{c.label}</option>
          ))}
