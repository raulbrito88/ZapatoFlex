...
export default async function AdminProductosPage() {
  ...
  const [productos, marcas, colores, tallas] = await Promise.all([
    prisma.producto.findMany({
      include: { marca: true, color: true, variantes: { include: { talla: true } } },
          ...
          {productos.map((p) => (
            <li key={p.id} className="card" style={{ marginBottom: "0.75rem" }}>
              <div style={{ padding: "1rem", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "0.5rem" }}>
                <div>
                  <strong>{p.nombre}</strong> — {p.marca.nombre} · {p.categoria} · ${Number(p.precio).toLocaleString("es-CO")}
                  <br />
